...
mutable struct ARMAModel
    ...
    function ARMAModel(p,q,roots_,poles,θcoef,ϕcoef,covarIV,expbases,expampls)
        ...
        new(p,q,roots_,poles,θcoef,ϕcoef,covarIV,expbases,expampls)
    end
end
...
function ARMAModel(θcoef::Vector, ϕcoef::Vector)
    ...
    ARMAModel(p,q,roots_,poles,θ,ϕ,covarIV,expbases,expampls)
end
...
function ARMAModel(roots_::AbstractVector, poles::AbstractVector, variance::Real)
    ...
    ARMAModel(p,q,roots_,poles,θcoef*scale,ϕcoef,covarIV,expbases,expampls*scale)
end
...
function ARMAModel(bases::AbstractVector, amplitudes::AbstractVector, covarIV::AbstractVector)
    ...
    ARMAModel(p,q,roots_,poles,θcoef,ϕcoef,gamma[1:max(p,q+1)],bases,amplitudes)
end
...
function model_psd(m::ARMAModel, freq::AbstractVector)
    z = exp.(-2im*pi *freq)
    N = length(freq)
    numer = fill(complex(m.θcoef[1]), N)
    for i=1:m.q
        numer .+= m.θcoef[i+1] * (z.^i)
    end
    denom = fill(complex(m.ϕcoef[1]), N)
    for i=1:m.p
        denom .+= m.ϕcoef[i+1] * (z.^i)
    end
    abs2.(numer ./ denom)
end
